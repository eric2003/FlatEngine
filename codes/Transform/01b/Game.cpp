...
void Game::DrawBoxTranslate( QGraphicsRectItem * rItem, float dx, float dy )
{
    ...
    qDebug() << "sceneBoundingRect:" << sbrect.left() << " " << sbrect.top()
        ...
        << " width() = " << sbrect.width() << " height() = " << sbrect.height();
...
void Game::DrawBoxRotateVersion2( QGraphicsRectItem * rItem, float delta )
{
    QRectF brect = rItem->boundingRect();
    QRectF sbrect = rItem->sceneBoundingRect();
    QRectF rect = rItem->rect();
    ...
    qDebug() << "center:" << center.x() << " " << center.y();
    qDebug() << "pos 000:" << rItem->pos().x() << " " << rItem->pos().y();
    qDebug() << "rotation 000:" << rItem->rotation();

    QPointF offset = rItem->sceneBoundingRect().center();

    QTransform transform;
    transform.translate(offset.x(),offset.y());
    transform.rotate(this->last_delta+delta);
    transform.translate(-offset.x(),-offset.y());
    rItem->setTransform(transform);
    this->last_delta += delta;

    qDebug() << "pos 111:" << rItem->pos().x() << " " << rItem->pos().y();
    qDebug() << "rotation 111:" << rItem->rotation();
    qDebug() << "rotate angle:" << this->last_delta;
}
...
void Game::Draw(QKeyEvent *event)
{
    ...
    switch ( event->key() )
    {
        ...
        case Qt::Key_Return:
        {
            ...
            this->DrawBoxRotateVersion2(this->box,delta);
            break;
        }
        default:
            break;
