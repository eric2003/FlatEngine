...
Game::Game()
{
    ...
    this->AddBoxTEST( QRect( 0, 0, 50, 50 ) );
    this->AddBoxTEST( QRect( 25, 0, 50, 50 ) );
...
void Game::AddBoxTEST( const QRectF & boxRect )
{
...
void Game::CalBoxTranslate(float dx, float dy)
{
    for ( int i = 0; i < this->itemList.size(); ++ i )
    {
        ...
        if ( i == 0 )
        {
            ...
            float x0 = rItem->sceneBoundingRect().center().x() + dx;
            float y0 = - rItem->sceneBoundingRect().center().y() - dy;
            FlatBody &flatBody = this->bodyList[ i ];
            qDebug() << "x0 = " << x0 << " y0 = " << y0;
            flatBody.position.x = x0;
            flatBody.position.y = y0;
            flatBody.transformUpdateRequired = true;
            qDebug() << "flatBody.position = " << flatBody.position.x 
                << " " << flatBody.position.y;
            qDebug() << "bodyList[ 0 ].position = " << this->bodyList[ 0 ].position.x 
                << " " << this->bodyList[ 0 ].position.y;
        }
    }
}
