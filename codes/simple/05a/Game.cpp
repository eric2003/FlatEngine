...
void Game::InitSingleBox()
{
    ...
    this->CalBoxTranslateRotate( 0, 0, alphad );
    this->ComputeBoxCollisions();
    this->AddFlatBodyToScene();
    this->DrawBoxTranslateRotate();
}

void Game::InitTwoBoxes()
{
    float alphad = 45;
    float alpha = ( alphad / 180.0 ) * std::numbers::pi;
    float sina = std::sin( alpha );
    ...
    //this->AddBoxAlpha( QRect( x0c, y0c, lenth, lenth ), Qt::blue, 0.0f );
    //this->AddBoxAlpha( QRect( x1c, y1c, lenth, lenth ), Qt::blue, 0.0f );
    this->CalBoxTranslateRotate( 0, 0, alphad );
    this->ComputeBoxCollisions();
    this->AddFlatBodyToScene();
    this->DrawBoxTranslateRotate();
...
void Game::CalBoxTranslateRotate(float dx, float dy, float drot)
{
    for ( int i = 0; i < this->bodyList.size(); ++ i )
    {
        if ( i == 0 )
        {
            ...
            flatBody.Move( dr );
            flatBody.Rotate(drot);
            flatBody.GetTransformedVertices();
        }
        else
        {
            FlatVector dr( 0.0f, 0.0f );
            FlatBody &flatBody = this->bodyList[ i ];
            flatBody.Move( dr );
            flatBody.Rotate(drot);
            flatBody.GetTransformedVertices();
...
void Game::DrawBoxTranslateRotate()
{
    for ( int i = 0; i < this->itemList.size(); ++ i )
    {
        ...
        QPointF offset;
        offset.setX( flatBody.position.x );
        offset.setY( - flatBody.position.y );

        float delta = flatBody.drot;
...
void Game::Draw(QKeyEvent *event)
{
    ...
    switch ( event->key() )
    {
        case Qt::Key_Left:
        {
            ...
            dy = 0.0f;
            drot = 0.0f;
            this->CalBoxTranslateRotate( dx, dy, drot );
            this->ComputeBoxCollisions();
            this->DrawBoxTranslateRotate();
            break;
        }
